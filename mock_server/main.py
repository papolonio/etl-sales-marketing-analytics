...
def _build_leads(n: int = 1250) -> list:
    ...
    for i in range(n):
        ...
        # Datas relativas ao dia atual para garantir sobreposicao com o filtro
        # padrao do dashboard (ultimos 90 dias). Janela de 120 dias cobre
        # tanto o periodo padrao quanto leads ainda em negociacao.
        lead_date = today - timedelta(days=rng.randint(0, 90))
...
def _build_contacts(n: int = 600) -> list:
    ...
    for i in range(n):
        ...
        contact_date = today - timedelta(days=rng.randint(0, 90))
        created      = int(datetime(contact_date.year, contact_date.month, contact_date.day).timestamp())
        contacts.append({
            "id":                  2000001 + i,
            "name":                f"{fn} {ln}",
            "first_name":          fn,
