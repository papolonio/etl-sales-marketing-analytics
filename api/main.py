...
Endpoints:
    GET /                                  health check
    GET /api/v1/roi                        todos os registros de ROI (paginado)
    GET /api/v1/roi/{campaign_id}          filtrado por campanha
    GET /api/v1/roi/ad/{ad_id}             filtrado por anuncio
    GET /api/v1/dashboard/kpis             KPIs do dashboard (interno)
    GET /api/v1/dashboard/evolucao-marketing  evolucao diaria (interno)
    GET /api/v1/dashboard/ranking-vendedores  ranking de vendedores (interno)
    GET /api/v1/public/sales               vendas ganhas (DaaS, requer X-API-Key)
    GET /api/v1/public/leads               leads (DaaS, requer X-API-Key)
"""
...
from routers import dashboard, client_api
...
app.include_router(client_api.router,  prefix="/api/v1/public")

# ── Constante legada (endpoints /api/v1/roi) ───────────────────────────────────
