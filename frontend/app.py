...
=================================
Consome a DaaS API (FastAPI) rodando em http://localhost:8001/api/v1/dashboard.
"""
...
with tab1:
    ...
    with col_esq:
        ...
        # Passa filtro de datas para o endpoint (API filtra fact_sales pelo periodo)
        ...
        if rank:
            ...
            # Taxa de Conversao: share da receita total do time
            ...
            # Data bars: barra suave no fundo das celulas numericas
            styled = (
                df_tbl.style
                .format({
                    "Receita (R$)":     "R$ {:,.2f}",
    ...
    with col_dir:
        ...
        # Mock reagente ao filtro de datas: receita escala proporcionalmente ao periodo
        _dias  = max((data_fim - data_inicio).days + 1, 1)
        _fator = round(_dias / 90, 4)  # 90 dias = baseline
